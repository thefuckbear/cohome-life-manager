        ...
        <div className="agreement-list">
          {agreements.map((item) => (
            <article className="agreement-row" key={item.title}>
              ...
              <div className="agreement-copy"><div><span className="tag">{item.category}</span><span className={`tag ${item.status === '已生效' ? 'tag--success' : 'tag--warm'}`}>{item.status === '已生效' ? <Check size={12} /> : <Clock3 size={12} />}{item.status}</span></div><h3>{item.title}</h3><p>{item.copy}</p><span>{item.confirmed} / 3 位室友已确认</span></div>
              {item.status === '待我确认' ? <PlaceholderButton feature="访客公约确认" variant="primary">阅读并确认</PlaceholderButton> : <PlaceholderButton feature={`${item.title}详情`} variant="ghost">查看 <ChevronRight size={15} /></PlaceholderButton>}
            </article>
          ))}
        </div>
      </section>
