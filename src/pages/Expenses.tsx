...
export function Expenses() {
  const [allSettled, setAllSettled] = useLocalStorageState('cohome:expenses:settled', false)

  const settleBills = () => {
    setAllSettled(true)
    console.info('[合住 CoHome] 一键结算演示已完成')
    triggerNotice('3 笔本月账单已更新，刷新页面后状态仍会保留。')
  }
  ...
  return (
    <div className="page module-page">
      ...
      <section className="metric-row">
        <article className="metric-card"><span className="metric-icon metric-icon--orange"><ArrowUpRight size={20} /></span><div><small>我需要支付</small><strong>{allSettled ? '¥0.00' : '¥126.50'}</strong><span>{allSettled ? '本月账单已结清' : '共 2 笔待结算'}</span></div></article>
      ...
      <section className="panel module-panel">
        <div className="panel__header"><div><h2>本月账单</h2><p>所有共同费用都在这里</p></div><div className="header-actions"><PlaceholderButton feature="账单筛选" variant="secondary"><Filter size={16} /> 筛选</PlaceholderButton><button type="button" className="button button--primary" onClick={settleBills} disabled={allSettled}><WalletCards size={16} /> {allSettled ? '已全部结清' : '一键结算'}</button></div></div>
        <div className="table-list">
          {bills.map((bill) => {
            const status = allSettled ? '已结清' : bill.status
            return (
            <article className="table-row" key={bill.title}>
              ...
              <span className={`tag ${status === '已结清' ? 'tag--success' : 'tag--warm'}`}>{status}</span>
